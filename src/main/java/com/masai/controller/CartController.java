...
//	
	...
	@GetMapping(value = "/cart0")
	public ResponseEntity<List<CartItem>> getCartProductHandler0(@RequestHeader("token")String token){
		return new ResponseEntity<>(cartService.getCartProduct0(token), HttpStatus.ACCEPTED);
	}
	...
	public ResponseEntity<Double> getCartTotalHandler(@RequestHeader("token")String token){
		return new ResponseEntity<>(cartService.getCartTotam(token), HttpStatus.ACCEPTED);
	}
